...
class RegisterProjectTest(
    ...
    fun `처음 등록하면 문제 저장소를 만들어 대기줄에 세우고 그 난이도로 프로젝트를 만든다`() {
        val result = registerProject(commandOf(memberId = "member-1", quizLevel = Depth.L2))
        ...
        project.quizLevel shouldBe Depth.L2
    ...
    fun `다른 회원이 같은 저장소를 등록하면 문제 저장소는 그대로 두고 프로젝트만 추가한다`() {
        val first = registerProject(commandOf(memberId = "member-1", quizLevel = Depth.L2))

        val second = registerProject(commandOf(memberId = "member-2", quizLevel = Depth.L3))
    ...
    fun `같은 회원이 난이도를 바꿔 다시 등록해도 기존 프로젝트를 그대로 둔다`() {
        ...
        val second = registerProject(commandOf(memberId = "member-1", quizLevel = Depth.L3))

        second.projectId shouldBe first.projectId
        projectRepository.count() shouldBe 1
        projectOf("member-1").quizLevel shouldBe Depth.L2
    }
    ...
    fun `문제를 낼 수 없다고 판정된 저장소면 프로젝트를 만들지 않고 거절 사유를 그대로 알린다`() {
        ...
        val exception = shouldThrow<BaseException> { registerProject(commandOf(memberId = "member-1", quizLevel = Depth.L2)) }
