 ...
 *
 * 둘을 묶어 두는 이유는 뒤에서 좌표를 되짚을 방법이 없기 때문입니다.
 * 해제 디렉터리 이름은 `{owner}-{name}-{sha}`인데 owner에 `-`가 들어가면 경계를 가를 수 없어,
 * 주소를 파싱한 수집 단계만이 좌표를 정확히 압니다. 여기서 놓치면 저장할 때 복구하지 못합니다.
 *
 * [root]는 절대 경로이고 파이프라인이 도는 동안만 씁니다 — 산출물에 실리면
 * 저장된 학습 세트가 특정 머신의 해제 위치에 묶입니다. 단계들은 상대 경로를 여기에 resolve 합니다.
 */
