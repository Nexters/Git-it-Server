 ...
 *
 * 저장소마다 하나뿐인 공용 도큐먼트라 여러 회원이 같은 문제 세트를 나눠 씁니다. 누가 학습하는지와
 * 회원별 난이도는 `Project`가 들고 있어, 회원이 늘어도 이 도큐먼트는 커지지 않습니다.
 *
 ...
 * 수집 단계가 확정해 [RepoCheckout]에 실어 보낸 값을 그대로 받습니다.
 *
 * [name]·[ownerImageUrl]·[starCount]·[techStacks]는 첫 등록 시점의 스냅샷이고 갱신되지 않습니다.
 * 목록 화면이 읽는 값이라, 별이 늘거나 이름이 바뀌어도 여기 값은 그대로입니다.
 */
...
// 대기줄을 상태+등록순으로 훑으므로 정렬까지 인덱스가 받아 줘야 매번 컬렉션을 훑지 않는다.
// idx_pending은 대기 상태의 옛 이름이다. 키가 같고 이름만 다른 인덱스는 만들 수 없어, 이름을 바꾸려면 운영 DB에서 먼저 떨어뜨려야 기동된다.
@CompoundIndex(
    name = "idx_pending",
...
class QuizRepo(
    ...
    /** 생성 대기줄에 선 시각이자 줄의 순서. 등록과 [retry]가 각각 갱신해, 재시도한 저장소는 맨 뒤에 섭니다. */
    var registeredAt: Instant = registeredAt
        private set

    // 클라이언트에게 그대로 내보내는 코드다. REJECTED에서만 채워진다.
     ...
     *
     * [githubRepoUrl]은 사용자가 적어 넣은 문자열 그대로라 스킴과 접미사(`/`·`.git`)가 제각각입니다.
     */
